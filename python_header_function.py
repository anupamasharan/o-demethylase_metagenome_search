def co_localised_operon_hunt():
    ...
    # Here you define a suitable regex depending on your unique header list by splitting the header characetrs so that a numerical list can be inputted in the loop, the example shown here depends on the sample files
    TBA = 'WP\_|\.1' 
...
#our golden list is ready, now we need to convert back to string form to get complete IDs
    ...
    prefix = 'WP_' #update as per your regex
    suffix = '.1'  #update as per your regex
    ...
    ls_CP_pairs = list(set([x for x in ls_CP_all if ls_CP_all.count(x) == 1]))
    ls_all_headers = [ls_CP_ID,ls_MT2_ID,ls_MT1_ID,ls_CP_1_ID,ls_CP_triplicates,ls_CP_pairs]
    
    big_header_list_df = pd.DataFrame(ls_all_headers, index = ['CP_paired_MT2', 'MT2_paired_CP', 'MT1_paired_CP', 'CP_paired_MT1', 'CP_triplicates', 'CP_pairs_only'])
    big_header_list_df.to_csv("all_co_localised_headers.csv")
    return(print("The file called all_co_localised_headers.csv is ready in your working directory"))
